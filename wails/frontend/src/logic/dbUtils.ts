...
export const getLocalMods = (): Promise<ModModel> => {
  return get(LOCAL_MODS_STORE_NAME).then((result) => result?.data || {})
}

export const saveLocalMods = async (data: ModModel) => {
  await clearStore(LOCAL_MODS_STORE_NAME)
  await insert(LOCAL_MODS_STORE_NAME, { id: 'localMods', data })
}
...
export const getModUseage = (): Promise<ModUseageModel> => {
  return get(MOD_USEAGE_STORE_NAME).then((result) => result?.data || {})
}

export const saveModUseage = async (useage: ModUseageModel) => {
  await clearStore(MOD_USEAGE_STORE_NAME)
  await insert(MOD_USEAGE_STORE_NAME, { id: 'modUseage', data: useage })
}
